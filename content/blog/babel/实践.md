...
其实挺简单的，难的是去了解 babel 如何改变代码（有个[网址](https://astexplorer.net/#)专门可以查看各种代码变成 AST 的样子），遍历 AST 的时候 path 对象上的方法有哪些（最后还是去翻了源码才大概了解了怎么解决我的需求），新建 AST 要对着[这里面](https://babeljs.io/docs/en/babel-types)的文档来，会用以前学编程语言的一些概念（表达式，声明语句等等），我也是模模糊糊有点懵，最后勉强完成了任务，节省了大量的人工替换代码的时间
